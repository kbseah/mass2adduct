#' Report closest-matching adduct type for a list of mass differences with absolute cutoff
#'
...
#' from a list of such potential adduct types, using a flat mass difference
#' cutoff instead of a proportional ppm cutoff. The built-in data sets
...
#' @param width numeric; the precision to which to match the mass value
#'        (default: 0.001)
#'
...
#' d.diff.annotate <- adductMatchFlat(d.diff,add=adducts2) # Report closest matches
#'
#' @export

adductMatchFlat.massdiff <- function(x, add=adducts, width=0.001) {
  indices <- vector()
  matches <- vector()
